...
- **Bulk row-selection + actions** on all three Allowlist tables (Users,
  ...
  another), `BulkBar.jsx` (the action bar rendered through `DataTable`'s
  opt-in `selectable`/`renderSelectionBar` props — every existing `DataTable`
  usage that doesn't pass `selectable` renders unchanged).
