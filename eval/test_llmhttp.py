"""Shared LLM transport contract (app/llmhttp.py).

This is the RED-phase spec for the provider-neutral transport helper that
unifies the copy-pasted POST logic currently in app/llm.py's `_chat`,
app/guard.py's `classify`, and app/critic.py's `review`.
...
No API key, no app.config import needed — this module tests app/llmhttp.py in
...
def test_non_json_200_response_raises_valueerror_and_propagates():
    ...
    critic.review), not in this transport helper — see eval/test_guard.py and
    eval/test_critic.py for those fail-open assertions."""
...
def run():
    print("shared LLM transport contract (app/llmhttp.py):")
