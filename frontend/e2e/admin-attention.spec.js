...
// Admin "attention" indicators: a total badge on the user-badge avatar (live on
...
test("avatar attention badge is live on Chat, and each section shows its count", async ({ page }) => {
  await adminMocks(page);
  await mockAttention(page, { users: 2, skills: 3, logs: 5 });
  await mockMarkLogsSeen(page);

  // On Chat — the avatar badge still reflects the total (proves the Shell-level
  // fetch, not a per-admin-tab one).
  await page.goto("/");
  await expect(page.getByRole("button", { name: "Account menu, 10 items need attention" })).toBeVisible();
  await expect(page.locator(".avatar-badge")).toHaveText("10");
...
test("viewing Logs acknowledges the problems: the Logs badge clears and the total drops", async ({ page }) => {
  ...
  // ...the Logs section badge is gone, and the avatar total drops to 5 (2+3+0).
  await expect(page.locator(".subtabs").getByRole("link", { name: "Logs" })).toHaveText("Logs");
  await expect(page.getByRole("button", { name: "Account menu, 5 items need attention" })).toBeVisible();
});
...
test("a large count collapses to a capped 99+ form", async ({ page }) => {
  ...
  await page.goto("/admin/users/current");
  await expect(page.locator(".avatar-badge")).toHaveText("99+");
...
test("no work waiting means no badge anywhere", async ({ page }) => {
  ...
  await page.goto("/admin/users/current");
  // The avatar advertises no attention (plain "Account menu", no count pill).
  await expect(page.getByRole("button", { name: "Account menu" })).toBeVisible();
  await expect(page.locator(".avatar-badge")).toHaveCount(0);
