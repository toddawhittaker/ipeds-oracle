...
on the dark theme's `--panel` background):
    ...
    convert brand/wordmark.png -resize x200 web/src/assets/wordmark.png
    convert brand/wordmark.png -fuzz 16% -fill '#e7edf6' -opaque '#1e3246' \
      -resize x200 web/src/assets/wordmark-dark.png
...
flattened on white since iOS shows black behind transparency):
    ...
    convert brand/icon.png -bordercolor none -border 6%x6% \
      -define icon:auto-resize=64,48,32,16 web/public/favicon.ico
    convert brand/icon.png -bordercolor none -border 6%x6% -resize 32x32 web/public/favicon-32.png
    convert brand/icon.png -bordercolor none -border 9%x9% -background white -flatten \
      -resize 180x180 -gravity center -extent 180x180 web/public/apple-touch-icon.png
